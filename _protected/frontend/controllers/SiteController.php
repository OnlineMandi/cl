...
//------------------------------------------------------------------------------------------------//
    ...
    {  
		...
		if (Yii::$app->request->isAjax && $model->load($_POST) && $profile->load($_POST) && $closet->load($_POST))
		{
			Yii::$app->response->format = 'json';
			$user_err = \yii\widgets\ActiveForm::validate($model);
			$profile_err = \yii\widgets\ActiveForm::validate($profile);
			$closet_err = array();
			if($profile->is_seller == 1)
				$closet_err = \yii\widgets\ActiveForm::validate($closet);
			$errors = array_merge($user_err,$profile_err,$closet_err);
			return $errors;
		}
        // collect and validate user data
        if ($model->load(Yii::$app->request->post()) && $closet->load(Yii::$app->request->post()) && $profile->load(Yii::$app->request->post()) && Model::validateMultiple([$model, $profile]))
        {
            ...
            if ($user = $model->signup()) 
            {
				...
				if($profile->is_seller == 1){
					...
					Role::deleteAll(['user_id' => $user->id]);
 ...
 *--------------------*/
    ...
    { 
		...
		$model = new Pages();
		$page = $model->find()->where(['slug'=>$slug,'status'=>1])->one();
